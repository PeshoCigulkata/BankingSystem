...
Task::Task(TaskType type, Client* client) :type(type), client(client) {

}
Task::Task(TaskType type, Client* _client, const String& accountID) :type(type), client(_client) {
	client->setAccountNumber(accountID);
}
Task::Task(const TaskType& type, Client* _client, const String& accountNumber, const String& nameBank) {
	this->type = type;
	client = _client;
